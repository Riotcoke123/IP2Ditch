...
# Communities.Win API URLs (comma-separated in env var)
DEFAULT_CW_API_URLS = [
...
if CW_API_URLS_STR:
    # Split and strip whitespace from each URL
...
# Data File Path
# Use a relative path as default if not set, assuming 'backup' subdir exists
DEFAULT_DATA_PATH = os.path.join('backup', 'data.json')
...
# Define supported file extensions (lowercase) - Keep these hardcoded as they define functionality
...
# --- Flask App ---
# Use the script's directory to find templates/static if APP_DATA_FILE_PATH is elsewhere
...
# --- Helper Functions (load_data, save_data, fetch_communities_data, upload_to_fileditch) ---
# No changes needed in the core logic of these functions, they use the globally defined
# COMMUNITIES_HEADERS, FILEDITCH_UPLOAD_URL, DATA_FILE_PATH etc. which are now loaded from env vars.
...
def _run_processing_cycle():
    ...
    with data_lock:
        ...
        all_posts_from_apis = []
        # Uses global COMMUNITIES_API_URLS loaded from env
        for url in COMMUNITIES_API_URLS:
            data = fetch_communities_data(url) # fetch uses global headers
            if data:
                posts_list = None
                if isinstance(data, list):
                    posts_list = data
                    # logging.info(f"Received list ({len(posts_list)} items) directly from {url}") # Reduce verbosity
                elif isinstance(data, dict):
                    # logging.debug(f"Received dictionary from {url}. Looking for posts list...") # Reduce verbosity
                    ...
                    for key in possible_keys:
                        ...
                        if isinstance(potential_list, list):
                            ...
                            found_key = key
                            # logging.info(f"Found list ({len(posts_list)} items) under key '{found_key}' in dict response from {url}") # Reduce verbosity
                            break
                    if posts_list is None and all(k in data for k in ['title', 'author', 'link']):
                        # logging.info(f"Treating root dictionary from {url} as a single post.") # Reduce verbosity
                        posts_list = [data]
                ...
                if posts_list is not None:
                    ...
                    # logging.info(f"Added {valid_posts_count} valid-structured posts from {url} to process queue.") # Reduce verbosity
            else:
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Log error but try to continue - Flask server might still run if data isn't essential initially
        logging.error(f"Could not create backup directory {backup_dir}: {e}. File operations might fail.")
        # Optionally exit if the data dir is absolutely critical:
        # logging.critical(f"Fatal: Could not create directory {backup_dir}: {e}. Exiting.")
        # exit(1)

    # --- Ensure templates/static directories exist relative to the script ---
    # Uses script_dir defined earlier where Flask app was initialized
    template_dir = app.template_folder # Use path set during Flask app init
    static_dir = app.static_folder     # Use path set during Flask app init
    ...
    # Use use_reloader=False because of the background thread
